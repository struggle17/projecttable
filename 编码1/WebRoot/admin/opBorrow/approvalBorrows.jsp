...
<body>
	...
	%><tr><th><%=i%></th><%
		...
		<th>
			<input type="button" value="详情" onclick="window.location.href('<%=basePath%>admin/index.jsp?mainPage=opBorrow/detailBorrow&page=approvalBorrows&clno=<%=getClnoFromRS(i)%>&u=<%=getUnoFromRS(i)%>&d=<%=getDateFromRS(i)%>&p=<%=getPeriodFromRS(i)%>')" style="width:50px;height:18px;background:#8888ff"/>
		</th>
		<th>
			<input type="button" value="批准" onclick="window.location.href('<%=basePath%>admin/index.jsp?mainPage=opBorrow/approvalOk&clno=<%=getClnoFromRS(i)%>&u=<%=getUnoFromRS(i)%>&d=<%=getDateFromRS(i)%>&p=<%=getPeriodFromRS(i)%>')" style="width:50px;height:18px;background:#8888ff"/>
		</th>
		<th>
			<input type="button" value="删除" onclick="window.location.href('<%=basePath%>admin/index.jsp?mainPage=opBorrow/delBorrow&page=approvalBorrows&clno=<%=getClnoFromRS(i)%>&u=<%=getUnoFromRS(i)%>&d=<%=getDateFromRS(i)%>&p=<%=getPeriodFromRS(i)%>')" style="width:50px;height:18px;background:#ff8888"/>
		</th>
		</tr>
	<%
	}%>
